...
3. Where is `data_preparation.R` and what is this? Why not just clean the data in one step?
4. What was changed from the raw qualtrics data for analyses? Looks like some deidentification was done.
5. Why use the position specifications for recoding ethnicity? (looks like results are the same without) (^|,)White/European(,|$)
6. Qualtrics data notes that timezones need to be taken care of, but then assigns Denver TZ later. Happy to help with this.. My understanding is that the timezone is set by the account that administered the survey.
7. Ideally, we'd read in the variable mappings for QT data rather than typing them manually
8. Should the 'alt' DVs have factor levels 'knows' and 'feels' rather than 'knows' and 'believes'?
...
d.sss <- d.sss.raw %>%
  ...
  mutate(across(ends_with(c("_know_bin", "_know_alt")),
         ...
         #this contradicts what is stated immediately before
         #however, it is correct and the statement above seems to be incorrect
