...
levels(full_long$education) <- c(levels(full_long$education), 20)
...
ds.model <- lme(know_vas ~ cond + comp + age + gender + education,
                random = list(~1|id, ~1|vignette, ~1|lab_id),
                data = final_long,
                na.action = "na.omit")
...
```{r}
model.2 <- glmer(know_vas_combined ~ (1|id),
                      data = final_long,
                      family = binomial,
...
```
  ...
  - Interpretation: the inclusion of participant ID isn't really useful, as the AIC is higher for this model. The variance between participants is very small.
  
### Model 3 = WHAT TO DO HERE 
...
coefficients <- coef(model.3) 
coef_DF <- as.data.frame(coefficients$`vignette:id`)
coef_DF$vig <- substr(rownames(coef_DF), 0, 3)
tapply(coef_DF$`(Intercept)`, coef_DF$vig, mean)

# log odds zero is the center 
exp(coef(model.3)$vignette) # regular odds with 1 at the center 
...
```
  ...
  - Therefore, we will continue with the random intercept for `vignette` as this parameter accounts for the variance in the DV. 
...
```{r}
model.5 <- glmer(know_vas_combined ~ (1|vignette) + 
...
table(model.5@frame$gender2, model.5@frame$know_vas_combined) #look at the used data 
...
```
  ...
  - Compensation: no effect
  - Age: no effect
  - Gender: Males and females did the task differently - men are slightly more likely to say someone believes something over knows 
  - Education: no effect 
  - These fixed effects account for very little variance in the model though.
...
```{r}
model.6 <- glmer(know_vas_combined ~ (1|vignette) + 
                   comp + age + gender2 + education + 
                   cond,
                      data = final_long,
                      family = binomial,
...
NEED TO FIX THE EDUCATION VARIABLE. 

```{r eval = F}
model.6A <- glmer(know_vas_combined ~ (1|vignette) + 
                    comp + age + gender2 + education + 
                    cond*turk,
                      data = final_long,
                      family = binomial,
...
```{r eval = F}
model.6B <- glmer(know_vas_combined ~ (1|vignette) + 
                    comp + age + gender2 + education + 
