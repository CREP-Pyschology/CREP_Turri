...
ggsave("../09_Figures/know_vas_problem.png",  dpi = 300, 
...
length(unique(final_long$person_code))
...
ggsave("../09_Figures/know_vas_problem2.png",  dpi = 300, 
       width = 8, height = 6, units = "in")

## knowledge 
ds.model <- lme(know_vas ~ cond + comp + age + gender + education,
                random = list(~1|vignette, ~1|id, ~1|person_code),
                data = final_long,
                na.action = "na.omit")
...
ggsave("../09_Figures/reason_vas_problem.png",  dpi = 300, 
       width = 8, height = 6, units = "in")

## reasonable 
ds.model <- lme(reason_vas ~ cond + comp_type + age + 
                  gender + education,
                random = list(~1|vignette, ~1|id), #won't converge with person_code
                data = final_long,
                na.action = "na.omit")
...
ggsave("../09_Figures/luck_vas_problem.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
...
```{r}
table(final_long$person_code, final_long$vignette, useNA = "ifany")

table(final_long$person_code, final_long$comp, useNA = "ifany") # 6ce754 and f6d5a9

table(final_long$person_code, final_long$gender2, useNA = "ifany")

table(final_long$person_code, final_long$cond, useNA = "ifany")

table(final_long$person_code, final_long$know_vas_combined, useNA = "ifany")

table(final_long$person_code, final_long$reason_vas_combined, useNA = "ifany")

table(final_luck$person_code, final_luck$luck_vas_combined, useNA = "ifany")

table(final_long$person_code, final_long$education, useNA = "ifany")

tapply(final_long$age, final_long$person_code, mean, na.rm = T)

full_long %>% 
  group_by(person_code) %>% 
  summarize(NA_age = sum(is.na(age)), NotNA_age = sum(!is.na(age)),
            NA_gender = sum(is.na(gender)), NotNA_gender = sum(!is.na(gender))) 
```
...
ggsave("../09_Figures/know_vas_reverse.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
...
ggsave("../09_Figures/know_bin_reverse.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
```
...
tapply(k.model.5@frame$age, k.model.5@frame$know_vas_combined, mean, na.rm = T)
tapply(k.model.5@frame$education, k.model.5@frame$know_vas_combined, mean, na.rm = T)
```

  - Model 4 AIC: `r AIC(k.model.4)`
  - Model 5 AIC: `r AIC(k.model.5)`
  - Interpretation: this model is better than the vignette random intercept only model.
  - Age is a negative predictor of knows/believes, such that as age increases, we are more likely to pick knows. 
  - Education is a positive predictor, so that as education increases we are more likely to pick believes.
  - The fixed effects are very small ~ .001. 
...
```
  ...
  - This effect accounts for 12-15% of the fixed effect variance. 
...
ggsave("../09_Figures/know_cond_vig.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
```
...
# prop tests
prop1 <- prop.test(t(k.d_table[2:1, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(k.d_table[2:1, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(k.d_table[2:1, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
         n = sum(t(k.d_table[2:1, c(1,3)])),
         r = 2, c = 2)
```
...
# prop tests
prop1 <- prop.test(t(k.e_table[2:1, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(k.e_table[2:1, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(k.e_table[2:1, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
         n = sum(t(k.e_table[2:1, c(1,3)])),
         r = 2, c = 2)
```
...
# prop tests
prop1 <- prop.test(t(k.g_table[2:1, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(k.g_table[2:1, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(k.g_table[2:1, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
         n = sum(t(k.g_table[2:1, c(1,3)])),
         r = 2, c = 2)
```
...
```{r}
k.model.6C <- glmer(know_vas_combined ~ (1|vignette/id/person_code) + 
                    comp + age + gender2 + education + 
...
table(r.model.5@frame$gender2, r.model.5@frame$reason_vas_combined)

tapply(r.model.5@frame$education, r.model.5@frame$reason_vas_combined, mean, na.rm = T)
...
```
  ...
  - Coefficients:
    ...
    - Gender - females are more likely to pick reasonable (13:1 odds) versus men (11:1 odds)
    - Education is a negative coefficient indicating that the less education a person listed, the more likely they were to pick reasonable. 
...
table(r.model.6@frame$cond, r.model.6@frame$reason_vas_combined)
...
ggsave("../09_Figures/reason_cond_vig.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
```
...
# prop tests
prop1 <- prop.test(t(r.d_table[2:1, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(r.d_table[2:1, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(r.d_table[2:1, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
         n = sum(t(r.d_table[2:1, c(1,3)])),
         r = 2, c = 2)
```
...
# prop tests
prop1 <- prop.test(t(r.e_table[2:1, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(r.e_table[2:1, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(r.e_table[2:1, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
         n = sum(t(r.e_table[2:1, c(1,3)])),
         r = 2, c = 2)
```
...
# prop tests
prop1 <- prop.test(t(r.g_table[2:1, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(r.g_table[2:1, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(r.g_table[2:1, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
         n = sum(t(r.g_table[2:1, c(1,3)])),
         r = 2, c = 2)
```
...
```{r}
r.model.6C <- glmer(reason_vas_combined ~ (1|vignette/id/person_code) + 
                    comp + age + gender2 + education + 
...
table(final_luck$vignette, final_luck$luck_vas_combined)
...
```
  ...
  - Coefficients: interpretation overall of random intercept, which effectively tells us regardless of condition, which way the participants lean toward answering. See the table printed out to help understand these values. 
    ...
    - Gerald: People are more even, but lean toward luck 
    - The results here indicate what we were seeing with the "continuous" data as well - that there's a big difference in the way they respond to vignettes
  - This vignette accounts for 8 to 10 percent of the random variance. 
...
tapply(l.model.5@frame$age, l.model.5@frame$luck_vas_combined, mean, na.rm = T)
tapply(l.model.5@frame$education, l.model.5@frame$luck_vas_combined, mean, na.rm = T)
...
```
  ...
  - Interpretation: this model is better than the vignette random intercept only model.
  - Coeffients:
    - Compensation: when people are compensated they are more likely to pick luck.   
    - Age: older individuals are more likely to pick ability. 
...
```
  ...
  - This effect accounts for 5-6% of the fixed effect variance. 
...
ggsave("../09_Figures/luck_cond_vig.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
```

  - Model 6 AIC: `r AIC(l.model.6)`
  - Model 6A AIC: `r AIC(l.model.6A)`
  - Interpretation: This model is better than a model without the interaction. 
  - There is an interaction between vignette and condition for choice. 
...
# prop tests
prop1 <- prop.test(t(l.d_table[1:2, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(l.d_table[1:2, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(l.d_table[1:2, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
...
# prop tests
prop1 <- prop.test(t(l.e_table[1:2, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(l.e_table[1:2, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(l.e_table[1:2, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
...
# prop tests
prop1 <- prop.test(t(l.g_table[1:2, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(l.g_table[1:2, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(l.g_table[1:2, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
         n = sum(t(l.g_table[1:2, c(1,3)])),
         r = 2, c = 2)
```
...
```{r}
l.model.6C <- glmer(luck_vas_combined ~ (1|vignette/id/person_code) + 
                    comp + age + gender2 + education + 
...
ggsave("../09_Figures/luck_vas_source.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
```
...
#HM getting error -- 
#Error in names(attr(result$observed, "dimnames")) <- c(dname_x, dname_y) : 
...
```{r}
## add to this data
final_luck$know_vas_binned <- final_luck$know_vas
final_luck$know_vas_binned[final_luck$know_vas_binned <= 40] <- 2
final_luck$know_vas_binned[final_luck$know_vas_binned > 40 & 
                             final_luck$know_vas_binned < 60] <- NA
final_luck$know_vas_binned[final_luck$know_vas_binned >= 60] <- 1
table(final_luck$know_vas_binned, useNA = "ifany")


final_luck$know_vas_combined <- ifelse(is.na(final_luck$know_vas_binned), 
                                     final_luck$know_bin, 
                                     final_luck$know_vas_binned)

final_luck$know_vas_combined <- final_luck$know_vas_combined - 1

final_luck$know_vas_combined <- 1 - final_luck$know_vas_combined

k.model.6E <- glmer(know_vas_combined ~ (1|vignette/id/person_code) + 
                    comp + age + gender2 + education + 
...
ggsave("../09_Figures/know_luck_vas.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
```
...
# prop tests
prop1 <- prop.test(t(l_table[2:1, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(l_table[2:1, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(l_table[2:1, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
         n = sum(t(l_table[2:1, c(1,3)])),
         r = 2, c = 2)
...
# prop tests
prop1 <- prop.test(t(a_table[2:1, 1:2]))
prop1
v.chi.sq(x2 = prop1$statistic, 
         n = sum(t(a_table[2:1, 1:2])),
         r = 2, c = 2)

prop2 <- prop.test(t(a_table[2:1, c(1,3)]))
prop2
v.chi.sq(x2 = prop2$statistic, 
         n = sum(t(a_table[2:1, c(1,3)])),
         r = 2, c = 2)
```
...
final_long$know_alt <- 1 - final_long$know_alt #rescore so same direction
...
k.model.6F <- glmer(know_alt ~ (1|vignette/id/person_code) + 
                   comp + age + gender2 + education + 
                   cond,
                      data = final_long,
                      family = binomial,
...
```{r}
k.model.6G <- glmer(know_vas_combined ~ (1|vignette/id/person_code) + 
                   comp + age + gender2 + education + 
...
ggsave("../09_Figures/know_vas_vig_mos.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
...
ggsave("../09_Figures/know_vas_cond_mos.png",  dpi = 300, 
       width = 8, height = 6, units = "in")
```
...
```{r}
k.model.6H <- glmer(know_vas_combined ~ (1|vignette/id/person_code) + 
                   comp + age + gender2 + education + 
...
# get people who missed darrel
darrel_exclude <- 
  c(full_long$id[full_long$vignette == "Darrel" & full_long$compr == 2 & full_long$cond != "Ignorance"], # not ignorance and 2
    full_long$id[full_long$vignette == "Darrel" & full_long$compr == 1 & full_long$cond == "Ignorance"])

length(unique(na.omit(darrel_exclude)))
    
full_long$darrel_exclusion <- FALSE
full_long$darrel_exclusion[full_long$id %in% darrel_exclude] <- TRUE

# percent correct
darrel_correct <- table(full_long$darrel_exclusion[!duplicated(full_long$id)]) / length(unique(full_long$id))

darrel_DF <- subset(full_long, darrel_exclusion == FALSE)
darrel_DF <- subset(darrel_DF, vignette_order == "DGE" | 
                      vignette_order == "DEG")
darrel_DF <- subset(darrel_DF, vignette == "Darrel")

nrow(darrel_DF)

# add to this DF
darrel_DF$know_vas_binned <- darrel_DF$know_vas
darrel_DF$know_vas_binned[darrel_DF$know_vas_binned <= 40] <- 2
darrel_DF$know_vas_binned[darrel_DF$know_vas_binned > 40 & 
                             darrel_DF$know_vas_binned < 60] <- NA
darrel_DF$know_vas_binned[darrel_DF$know_vas_binned >= 60] <- 1
darrel_DF$know_vas_combined <- ifelse(is.na(darrel_DF$know_vas_binned), 
                                     darrel_DF$know_bin, 
                                     darrel_DF$know_vas_binned)

darrel_DF$know_vas_combined <- 3 - darrel_DF$know_vas_combined

darrel_only <- chisq.test(darrel_DF$cond, darrel_DF$know_vas_combined)
MOTE_darrel <- v.chi.sq(
  x2 = darrel_only$statistic,
  n = sum(darrel_only$observed),
  r = 2,
  c = 3,
  a = .05
)

MOTE_darrel$estimate
MOTE_darrel$statistic
```
...
lab_sheet_temp <- lab_sheet %>% 
  filter(unique_project != "NANA")

# number of projects that applied that have a link person code (65)
lab_sheet_temp %>% 
  filter(!duplicated(unique_project)) %>% 
  nrow()

# number of projects that got the protocol approved (51)
lab_sheet_temp %>% 
  filter(protocol_approved == "Yes") %>% 
  filter(!duplicated(unique_project)) %>% 
  nrow()

# therefore the subtraction is how many were not approved (14)

# number of unique projects who were approved and contributed data (43)
lab_sheet_temp %>% 
  filter(use == "Yes") %>% 
  filter(protocol_approved == "Yes") %>% 
  filter(!duplicated(unique_project)) %>% 
...
# therefore 8 were lost (51 - 43) because they either didn't collect data
# or we could not use the qualtrics data 

# number of project who "finished" crep
lab_sheet_temp %>% 
  filter(protocol_approved == "Yes") %>% 
  filter(completed_crep_requirements == "Yes") %>% 
  filter(!duplicated(unique_project)) %>% 
  nrow()

# number of labs who had data in the full sample
full_long_project <- full_long %>% 
  left_join(lab_sheet %>% select(lab_code, person_code, unique_project), 
            by = c("lab_id" = "lab_code", "person_code" = "person_code"))

length(unique(full_long_project$unique_project)) # match yay

final_long_project <- final_long %>% 
  left_join(lab_sheet %>% select(lab_code, person_code, unique_project), 
            by = c("lab_id" = "lab_code", "person_code" = "person_code"))
...
THAT_TABLE <- 
  ...
  filter(use == "Yes") %>% 
  group_by(person_code) %>% 
  summarize(Language = paste(language, collapse = "_"),
            GP_Region = paste(country, collapse = "_"),
            Site = NA, 
            Teams = length(unique(unique_project)), 
            Sample = paste(sampled_population, collapse = "_"), 
            In_Person = paste(in_person, collapse = "_"), 
            In_Group = paste(group_data, collapse = "_"), 
            Compensation = paste(compensation, collapse = "_"), 
            Site_Analysis = NA)

FULL_N <- 
  full_long_project %>% 
  group_by(person_code) %>% 
  summarize(Full_N = n()/3)

FINAL_N <-
  final_long_project %>% 
  group_by(person_code) %>% 
  summarize(Final_N = n()/3)

THAT_TABLE <- THAT_TABLE %>% 
  left_join(FULL_N, by = c("person_code" = "person_code")) %>% 
  left_join(FINAL_N, by = c("person_code" = "person_code"))

flextable(THAT_TABLE)

FULL_N_project <- 
  full_long_project %>% 
  group_by(person_code, unique_project) %>% 
  summarize(Full_N = n()/3)
...
flextable(FULL_N_project %>% 
            left_join(FINAL_N_project, 
                      by = c("person_code" = "person_code", 
