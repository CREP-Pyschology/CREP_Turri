...
*Luck Attributions*
  ...
  A model with a random intercept for vignette nested within participant (Model 3, AIC = `r round(AIC(l.model.3), 2)`) explained similar amounts of variance as the previous model (Pseudo-_R_^2^ = `r round(r.squaredGLMM(l.model.3)[2,2], 3)`% - `r round(r.squaredGLMM(l.model.3)[1,2], 3)`). The model with a random intercept for vignette nested in participant nested in data collection site (Model 4, AIC = `r AIC(l.model.4)`) accounted for the same proportion of random variance (Pseudo-_R_^2^ =`r round(r.squaredGLMM(l.model.4)[2,2], 3)`% to `r round(r.squaredGLMM(l.model.4)[1,2], 3)`).
  
  [HM stopped here]
  
  
  Next, covariates were added to the model as fixed effects (Model 5, AIC = `r round(AIC(l.model.5), 2)`). Relative to the model without covariates, this model was more useful in explaining variance in knowledge attribution. There was an association between years of education and luck attributions. People who had fewer years of education were more likely to attribute luck rather than ability.
