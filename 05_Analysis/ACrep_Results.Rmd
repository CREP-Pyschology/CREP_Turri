...
final_demographics %>% 
  ...
  select(-ethn_other_text,
         -ethn_none) %>%  #Mturk and Portuguese labs added a 'prefer not to say' response option
  mutate(comp = comp - 1,
         `centralized survey` = source - 1) %>%  #adjusting numeric values so 0 is no and 1 is yes
  select(-source, -ethn_selected) %>% 
  rename(mturk = turk) %>% 
  describe() %>% 
  .[,c(2,3,4,8,9)] %>% 
  mutate(count = n*mean,
...
final_demographics %>% 
  ...
  kable(digits = 2, caption = "Number and Percent of Participants by Data Collection Context Variables") %>% 
  kable_styling()

comp_missing_lab <- final_demographics %>% 
  filter(is.na(comp)) %>% 
  pull(lab_id)

final_demographics %>% 
  filter(lab_id == comp_missing_lab) %>% 
  tabyl(comp)
