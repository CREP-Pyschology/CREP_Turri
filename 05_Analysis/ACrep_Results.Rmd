...
use_data %>% 
  select(un_region, compensation, sampled_population, group_data, 
         in_person, deviation_issues, full_sample_size, 
         final_sample_size) %>% 
...
## Analytic Approach
  ...
  All raw data collected for this study were aggregated and analyzed as a single data set. Multilevel models were used to evaluate hypotheses. The unit of analysis was the trial and random intercepts for the vignette, participant, and data collection site were be included to account for the nesting of trials within these groups. 
...
## Sample Size Determination
  ...
  The approved protocol described power analyses. The text from the original protocol is reproduced in full in Appendix X, and is summarized here. Power analyses suggested that, given 3 vignettes per participant and 9 labs, at least 32 participants per lab (288 total participants; 864 total observations), would be required to detect an effect of X size at a power of at least .90 with an alpha of .05.
...
# Results
  ...
  In the approved protocol, the results section focused heavily on the project's logistics and structured results reporting in ways that would not allow for a transparent and thorough description of model fit and other important aspects of results, like assumption checks. Further, some model specification details in the approved protocol conflicted with stated research questions. We have implemented models that test the stated research questions. The results as they appeared in the approved protocol are included in full, with updated statistics where possible, in the Appendix.
  There were `r length(unique(final_long$person_code))` data collection sites in the final data set. The number of participants at each site varied (see Table X). In the approved protocol, we anticipated that all data collection sites would contribute data from at least 50 participants, after exclusions. `r final_long %>% group_by(person_code, id) %>% count() %>% group_by(person_code) %>% count() %>% filter(n < 50) %>% nrow()` sites fell short of this threshold.
  The approved protocol described testing assumptions before conducting analyses (for full results of the following assumption checks, see Appendix X). Assumptions of and related to linearity are primarily relevant for the analyses of the continuously measured dependent variables. The continuous knowledge attribution was dramatically bi-modal when examined unconditionally and by vignette and condition.

  A linear model that predicted continuously measured knowledge attribution as a function of condition, with covariates of compensation, age, gender, and education, produced a bi-modal residual distribution, indicating a violation of the residual normality assumption. Further, a plot of residuals by fitted values suggests that residuals vary as a function of predicted values, suggesting a violation of the homoscedasticity assumption. Similar patterns of results suggested that the assumptions of linear regression were also not met for continuously measured reasonableness or luck.

  It is generally not recommended to transform continuous variables in to discreet variables for analyses (cite). For the present analyses, however, splitting the continuously measured versions of dependent variables such that scores at and below 40 and scores at and above 60 were classified into a discreet category allows us to validly interpret model results and also test whether the method of measurement (continuous or binary) affects judgments. So, this was the approach taken.

*Knowledge attributions*

To determine whether participants made different attributions about vignette subject's knowledge as a function of the vignette's knowledge condition, we estimated a series of logistic regressions. We analyzed responses originally reported on continuous and binary scales together, using a variable where 0 means the subject's identification was attributed to their knowledge ("knows") and 1 means the subject's identification was not ("belief"). List-wise deletion was used to handle missing data. 
  First, a baseline intercept-only model was estimated. Then, models with random intercepts for vignette, person_code, and data collection site were added sequentially, before models with different interaction terms were estimated.
  Compared to the baseline model (AIC = `r round(AIC(k.model.1), 2)`), a model with a random intercept for vignette (AIC = `r round(AIC(k.model.2), 2)`) explained more variance. Pseudo Rsqaured values suggest that vignette accounts for `r round(r.squaredGLMM(k.model.2)[2,2]*100, 1)`% to `r round(r.squaredGLMM(k.model.2)[1,2]*100, 1)`% of the random variance in knowledge attributions. People tended to attribute knowledge differently in each vignette. People attributed Darrel's identification to his knowledge and his belief with roughly equal frequency, but slightly favored attributing his knowledge. People tended to attribute Emma's identification to her belief much more often than to her knowledge. People tended to attribute Gerald's identification to his belief slightly more often than to his knowledge. 
...
```{r all random intercepts on knowledge}
tidy(k.model.5) %>% 
  clean_variable_names() %>% 
...
In the Darrel vignette, people made different attributions in the different conditions X^2(`r k.dcond_effect$parameter[[1]]`) = `r round(k.dcond_effect$statistic[[1]], 2)`, p `r if(round(k.dcond_effect$p.value[[1]], 4) == 0)paste("<.0001")`. The likelihood that people attributed Darrel's identification to his knowledge differed in the ignorance and Gettier conditions such that people were more likely to attribute knowledge in the Gettier condition (`r round(k.dcond_GI$estimate1, 2)`) than in the ignorance condition (`r round(k.dcond_GI$estimate2, 2)`; Cramer's `r k.dcond_GI_v$estimate`, `r k.dcond_GI_v$statistic`). The likelihood that people attributed Darrel's identification to his knowledge also differed in the reasonableness and Gettier conditions such that people were more likely to attribute knowledge in the knowledge condition (`r round(k.dcond_GK$estimate2, 2)`than in the Gettier condition (`r round(k.dcond_GK$estimate1, 2)`; Cramer's `r k.dcond_GK_v$estimate`, `r k.dcond_GK_v$statistic`). 
...
*Reasonableness attributions*
  ...
  To determine whether participants made different judgments about vignette subjects' reasonableness as a function of the vignette's knowledge condition, we estimated a series of logistic regressions, just as with the previous analyses that focused on knowledge attribution as a dependent variable. Like with those analyses, we analyzed responses originally reported on continuous and binary scales together, using a variable where 0 means the subject was rated as reasonable and 1 means the subject was rated as unreasonable.  List-wise deletion was used to handle missing data. 
  A baseline intercept-only model was estimated initially and models with random intercepts for vignette, person_code, and data collection site were added sequentially, followed by a model with covariates, and then a model with knowledge condition. Then, models with interactions were estimated to better understand whether the effect of condition on reasonableness differed as a function of vignette, sampling type, and measurement type.
  Compared to the baseline model (AIC = `r round(AIC(r.model.1), 2)`), a model with a random intercept for vignette (AIC = `r round(AIC(r.model.2), 2)`) explained more variance. For all vignettes, the great majority of people rated subjects as reasonable. However, the rates differed slightly by vignette. Collapsing across conditions, more people rated Emma unreasonable than Gerald, and more people rated Gerald as unreasonable than Darrel. Pseudo Rsqaured values suggest that vignette accounts for a small portion of the random variance in reasonableness attributions (between `r round(r.squaredGLMM(r.model.2)[2,2]*100, 1)`% to `r round(r.squaredGLMM(r.model.2)[1,2]*100, 1)`%).
...
```{r random intercept for vignette, reasonableness}
tidy(r.model.2) %>% 
  clean_variable_names() %>% 
  kable(caption = "Explaining Variance in Reasonableness with a Random Intercept for Vignette") %>% 
  kable_styling()
```

  A model with a random intercept for vignette nested within participant (AIC = `r round(AIC(r.model.3), 2)`) explained similar amounts of variance to a model with a random intercept for vignette alone. Pseudo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(r.model.3)[2,2]*100, 1)`% to `r round(r.squaredGLMM(r.model.3)[1,2]*100, 1)`% of the random variance in knowledge attributions.
  Next, a model with a random intercept for vignette nested in participant nested in data collection site was estimated (AIC = `r AIC(r.model.4)`). The addition of data collection site was not useful relative to the previous model. Pseudo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(r.model.4)[2,2]*100, 1)`% to `r round(r.squaredGLMM(r.model.4)[1,2]*100, 1)`% of the random variance in knowledge attributions.
  
  Next, covariates were added to the model as fixed effects (AIC = `r round(AIC(r.model.5), 2)`). Relative to the model without covariates, this model was more useful in explaining variance in knowledge attribution. Compensation was associated with reasonableness; participants who were compensated were more likely to judge the subject as reasonable. Education was also associated with reasonableness. The fewer years of education people had the more likely they were to judge the subject as reasonable.
  
```{r reasonableness by random intercepts}
tidy(r.model.5) %>% 
  clean_variable_names() %>% 
  kable(caption = "Explaining Variance in Reasonableness with Random Intercepts for Vignette/Participant/Site and Covariates") %>% 
  kable_styling()
```

  Finally, a model was estimated that included knowledge condition as a fixed effect (AIC = `r round(AIC(r.model.6), 2)`). This model performed better than the previous model, suggesting that there was an effect of vignette condition on reasonableness. Although in all conditions, most people rated the subject as reasonable, both the knowledge condition and the ignorance condition differed from the Gettier condition (see Table X). In the ignorance condition, people were more likely to rate the subject reasonable than in the Gettier condition. In the knowledge condition, people were less likely to rate the subject reasonable than in the Gettier condition. Pseudo Rsqaured values suggest that condition accounts for `r round(r.squaredGLMM(r.model.6)[2,1]*100, 1)`% to `r round(r.squaredGLMM(r.model.6)[1,1]*100, 1)`% of the fixed effect variance in knowledge attributions.

```{r reasonableness by knowledge condition}
tidy(r.model.6) %>% 
  clean_variable_names() %>% 
  kable(caption = "Effect of Condition on Knowledge Attributions") %>% 
  kable_styling()
...
*Does the effect of knowledge condition differ by vignette?*
  ...
  To test whether the effect of condition varied by vignette, a model was estimated that included an interaction between vignette and subject (AIC = `r round(AIC(r.model.6A), 2)`). This model explained more variance than the model without the interaction term. As shown in Figure X, although the general pattern is the same for all vignettes, the magnitudes of differences varied by vignette. Pseudo Rsqaured values suggest that the interaction between condition and vignette accounts for `r round(r.squaredGLMM(r.model.6A)[2,1]*100, 1)`% to `r round(r.squaredGLMM(r.model.6A)[1,1]*100, 1)`% of the random variance in knowledge attributions.
...
In the Darrel vignette, people made different attributions in the different conditions X^2(`r r.dcond_effect$parameter[[1]]`) = `r round(k.dcond_effect$statistic[[1]], 2)`, p `r if(round(r.dcond_effect$p.value[[1]], 4) == 0)paste("<.0001")`. The likelihood that people rated Darrel reasonable differed in the ignorance and Gettier conditions such that people were more likely to rate Darrel reasonable in the Gettier condition (`r round(r.dcond_GI$estimate1, 2)`) than in the ignorance condition (`r round(r.dcond_GI$estimate2, 2)`; Cramer's `r r.dcond_GI_v$estimate`, `r r.dcond_GI_v$statistic`). The likelihood that people rated Darrel reasonable did not differ in the knowledge and Gettier conditions (Cramer's `r r.dcond_GK_v$estimate`, `r r.dcond_GK_v$statistic`). 
...
In the Emma vignette, people rated Emma's reasonableness differently across conditions X^2(`r r.econd_effect$parameter[[1]]`) = `r round(r.econd_effect$statistic[[1]], 2)`, p = `r round(r.econd_effect$p.value[[1]], 4)`. The likelihood that people rated Emma reasonable did not differ in the ignorance and Gettier conditions (Cramer's `r r.econd_GI_v$estimate`, `r r.econd_GI_v$statistic`). The likelihood that people rated Emma reasonable differed in the the knowledge and Gettier conditions such that people were more likely to rate Emma reasonable in the knowledge in the knowledge condition (`r round(r.econd_GK$estimate2, 2)`) than in the Gettier condition (`r round(r.econd_GK$estimate1, 2)`; Cramer's`r r.econd_GK_v$estimate`, `r r.econd_GK_v$statistic`). 
...
In the Gerald vignette,the overall pattern was the same. People made different attributions in the different conditions X^2(`r r.gcond_effect$parameter[[1]]`) = `r round(r.gcond_effect$statistic[[1]], 2)`, p = `r round(r.gcond_effect$p.value[[1]], 3)`. People were more likely to rate Gerald reasonable in the Gettier condition (`r round(r.gcond_GI$estimate1, 2)`) than in the ignorance condition (`r round(r.gcond_GI$estimate2, 2)`; Cramer's `r r.gcond_GI_v$estimate`, `r r.gcond_GI_v$statistic`). There was no difference between the knowledge and Gettier conditions (`r round(r.gcond_GK$estimate1, 2)`; Cramer's `r r.gcond_GK_v$estimate`, `r r.gcond_GK_v$statistic`). 

*Does the effect of knowledge condition differ in participants recruited through Mturk?*

To test whether the effect of condition on reasonableness differed as a function of how participants were recruited, we estimated a model that included an interaction between condition and recruitment type (AIC = `r round(AIC(r.model.6B), 2)`). As with knowledge attributions, this model performed better than a model without the condition by recruitment type interaction, but the interaction between condition and recruitment type was not significant. There was no evidence that the effect of condition differed as a function of how participants were recruited.

```{r reasonableness mturk interaction}
tidy(r.model.6B) %>% 
  clean_variable_names() %>% 
  kable(caption = "Interaction of Condition and Participant Recruitment Type on Reasonableness") %>% 
  kable_styling()
```
...
To test whether the effect of condition on reasonableness differed as a function of measurement type, we estimated a model that included an interaction between condition and measurement type (AIC = `r round(AIC(r.model.6C), 2)`). As with knowledge attributions, this model did not perform better than a model without the interaction. There was no evidence that the effect of condition differed depending on how reasonableness was measured.

*Luck*

  We estimated a series of logistic regressions. We analyzed responses originally reported on continuous and binary scales together, using a variable where 0 means the subject's identification was attributed to their luck and 1 means the subject's identification was attributed to their ability. List-wise deletion was used to handle missing data. 
  First, a baseline intercept-only model was estimated. Then, models with random intercepts for vignette, person_code, and data collection site were added sequentially. 
  Compared to the baseline model (AIC = `r round(AIC(l.model.1), 2)`), a model with a random intercept for vignette (AIC = `r round(AIC(l.model.2), 2)`) explained more variance. People tended to attribute luck differently in each vignette. For Darrel, people were more likely to attribute ability over luck. For Emma, people were more likely to attribute luck over ability. For Gerald, people showed a slight tendency to attribute luck over ability. Pseudo Rsqaured values suggest that vignette accounts for `r round(r.squaredGLMM(l.model.2)[2,2]*100, 1)`% to `r round(r.squaredGLMM(l.model.2)[1,2]*100, 1)`% of the random variance in luck attributions. 
  
```{r luck model vignette}
tidy(l.model.2) %>% 
  clean_variable_names() %>% 
  kable(caption = "Explaining Variance in Luck Attributions with a Random Intercept for Vignette") %>% 
  kable_styling()
```

  A model with a random intercept for vignette nested within participant (AIC = `r round(AIC(l.model.3), 2)`) explained similar amounts of variance to a model with a random intercept for vignette alone. Pseudo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(l.model.3)[2,2]*100, 1)`% to `r round(r.squaredGLMM(l.model.3)[1,2]*100, 1)`% of the random variance in knowledge attributions.
  Next, a model with a random intercept for vignette nested in participant nested in data collection site was estimated (AIC = `r AIC(l.model.4)`). The addition of data collection site was not useful relative to the previous model. Pseudo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(l.model.4)[2,2]*100, 1)`% to `r round(r.squaredGLMM(l.model.4)[1,2]*100, 1)`% of the random variance in knowledge attributions.
  Next, covariates were added to the model as fixed effects (AIC = `r round(AIC(l.model.5), 2)`). Relative to the model without covariates, this model was more useful in explaining variance in knowledge attribution. There was an association between years of education and luck attributions. People who had fewer years of education were more likely to attribute luck rather than ability.
...
```
  ...
  Finally, a model was estimated that included knowledge condition as a fixed effect (AIC = `r round(AIC(l.model.6), 2)`). This model performed better than the previous model, suggesting that there was an effect of vignette condition on people's luck attributions. Both the knowledge condition and the ignorance condition differed from the Gettier condition (see Table X). In the knowledge condition and the ignorance condition, people were more likely to attribute subject's identification to their ability. In the Gettier condition, people were more likely to attribute luck. Pseudo Rsqaured values suggest that condition accounts for `r round(r.squaredGLMM(l.model.6)[2,1]*100, 1)`% to `r round(r.squaredGLMM(l.model.6)[1,1]*100, 1)`% of the fixed effect variance in luck attributions.
...
*Does the effect of knowledge condition differ by vignette?*
  ...
  To better understand whether the effect of condition varied as a function of the vignette's content, a model was estimated that included an interaction between vignette and subject (AIC = `r round(AIC(l.model.6A), 2)`). This model explained more variance than the model without the interaction between condition and vignette. As shown in Figure X, each vignette shows a different pattern of condition effects. Pseudo Rsqaured values suggest that the interaction between condition and vignette accounts for `r round(r.squaredGLMM(l.model.6A)[2,1]*100, 1)`% to `r round(r.squaredGLMM(l.model.6A)[1,1]*100, 1)`% of the random variance in knowledge attributions.
...
In the Darrel vignette, people made different attributions in the different conditions X^2(`r l.dcond_effect$parameter[[1]]`) = `r round(l.dcond_effect$statistic[[1]], 2)`, p `r if(round(l.dcond_effect$p.value[[1]], 4) == 0)paste("<.0001")`. The likelihood that people attributed Darrel's identification to his luck did not differ in the ignorance and Gettier conditions (Cramer's `r l.dcond_GI_v$estimate`, `r l.dcond_GI_v$statistic`). The likelihood that people attributed Darrel's identification to his luck differed in the reasonableness and Gettier conditions. People were less likely to attribute reasonableness over luck in the knowledge condition (`r round(l.dcond_GK$estimate2, 2)`than in the Gettier condition (`r round(l.dcond_GK$estimate1, 2)`; Cramer's `r l.dcond_GK_v$estimate`, `r l.dcond_GK_v$statistic`). 
...
In the Emma vignette, people also made different attributions in the different conditions X^2(`r l.econd_effect$parameter[[1]]`) = `r round(l.econd_effect$statistic[[1]], 2)`, p `r if(round(l.econd_effect$p.value[[1]], 4) == 0)paste("<.0001")`. The likelihood that people attributed Emma's identification to her ability versus her luck differed in the ignorance and Gettier conditions such that people were more likely to attribute luck in the Gettier condition (`r round(l.econd_GI$estimate1, 2)`) than in the ignorance condition (`r round(l.econd_GI$estimate2, 2)`; Cramer's `r l.econd_GI_v$estimate`, `r l.econd_GI_v$statistic`). The likelihood of luck attribution also differed in the knowledge and Gettier conditions such that people were less likely to attribute luck in the knowledge condition (`r round(l.econd_GK$estimate2, 2)`) than in the Gettier condition (`r round(l.econd_GK$estimate1, 2)`; Cramer's`r l.econd_GK_v$estimate`, `r l.econd_GK_v$statistic`). 
...
In the Gerald vignette,the overall pattern was the same. People made different attributions in the different conditions X^2(`r l.gcond_effect$parameter[[1]]`) = `r round(l.gcond_effect$statistic[[1]], 2)`, p `r if(round(l.gcond_effect$p.value[[1]], 4) == 0)paste("<.0001")`. As with Darrel and Emma, the likelihood that people attributed Gerald's identification to his luck differed such that people were more likely to attribute luck in the Gettier condition (`r round(l.gcond_GI$estimate1, 2)`) than in the ignorance condition (`r round(l.gcond_GI$estimate2, 2)`; Cramer's `r l.gcond_GI_v$estimate`, `r l.gcond_GI_v$statistic`). In addition, people were less likely to attribute luck to Gerald in the knowledge condition (`r round(l.gcond_GK$estimate2, 2)`) than in the Gettier condition (Cramer's `r l.gcond_GK_v$estimate`, `r l.gcond_GK_v$statistic`). 

*Does the effect of knowledge condition differ in participants recruited through Mturk?*

We estimated a model that included an interaction between condition and recruitment type (AIC = `r round(AIC(l.model.6B), 2)`). This model performed better than a model without the condition by recruitment type interaction. However, as with models predicting knowledge attributions and reasonableness, the interaction between condition and recruitment type was not significant. There was no evidence that the effect of condition differed as a function of how participants were recruited.

```{r luck mturk interaction}
tidy(l.model.6B) %>% 
  clean_variable_names() %>% 
  kable(caption = "Interaction of Condition and Participant Recruitment Type on Luck") %>% 
  kable_styling()
```
...
We estimated a model that included an interaction between condition and measurement type (AIC = `r round(AIC(l.model.6C), 2)`). This model performed better than a model without the condition by measurement type interaction. As shown in Figure X, the effect was consistent across measurement type, suggesting a slight difference in magnitude between the two measurement types for one or more conditions.
...
For the binary measurement type, there was an effect of condition on luck attributions  X^2(`r bin_cond$parameter[[1]]`) = `r round(bin_cond$statistic[[1]], 2)`, p `r if(round(bin_cond$p.value[[1]], 4) == 0)paste("<.0001")`. People were more likely to attribute subjects' identifications to luck in the Gettier condition (`r round(bin_cond_GI$estimate1, 2)`) than in the ignorance condition (`r round(bin_cond_GI$estimate2, 2)`; Cramer's `r bin_cond_GI_v$estimate`, `r bin_cond_GI_v$statistic`). People were also more likely to attribute subjects' identifications to luck in the Gettier condition (`r round(bin_cond_GK$estimate1, 2)`) than in the knowledge condition (`r round(bin_cond_GK$estimate2, 2)`; Cramer's `r bin_cond_GK_v$estimate`, `r bin_cond_GK_v$statistic`). 
...
For the continuous measurement type, there was a similar pattern of results. There was an effect of condition on luck attributions  X^2(`r vas_cond$parameter[[1]]`) = `r round(vas_cond$statistic[[1]], 2)`, p `r if(round(vas_cond$p.value[[1]], 4) == 0)paste("<.0001")`. People were more likely to attribute subjects' identifications to luck in the Gettier condition (`r round(vas_cond_GI$estimate1, 2)`) than in the ignorance condition (`r round(vas_cond_GI$estimate2, 2)`; Cramer's `r vas_cond_GI_v$estimate`, `r vas_cond_GI_v$statistic`). People were also more likely to attribute subjects' identifications to luck in the Gettier condition (`r round(vas_cond_GK$estimate1, 2)`) than in the knowledge condition (`r round(vas_cond_GK$estimate2, 2)`; Cramer's `r vas_cond_GK_v$estimate`, `r vas_cond_GK_v$statistic`). The effect sizes of the effects were smaller when luck was measured continuously, but the confidence intervals of these effect sizes overlapped with the confidence intervals of the effect sizes for the binary measurement type. 

### Exploratory analyses

[Getting more information about why re ran these]
