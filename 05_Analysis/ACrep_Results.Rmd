...
*Education level*
  ...
  Participants were asked to report their gender, and were given three response options. The vast majority of participants selected "male" or "female" (_n_ identified as neither = `r final_demographics`)
...
## Power Anlaysis
  ...
  The approved protocol described power analyses. The text from the original protocol is reproduced in full in Appendix X, and is summarized here. 
  Power analyses were conducted to estimate the number of participants required to estimate an effect of knowledge condition on participants knowledge attributions at 90% power with an alpha of .05, and effect sizes based on the pilot test, Experiment 1 of Turri et al. (2015), and simulations of different model specifications and samples based on the pilot data. The effect sizes used were as follows: For the difference between the Gettier condition and the ignorance condition, we estimated the sample size required to detect a standardized fixed effect of .3. For the difference between the Gettier condition and the knowledge control condition, we estimated the sample size required to detect a standardized fixed effect of .1. 
  Power anlyses suggested that, given 3 vignettes per participant and 9 labs, at least 32 participants per lab (288 total participants; 864 total observations), would be required to achieve a power of at least .90 with an alpha of .05.
  ...
  [ Erin -- can you check / correct this section? I don't really understand what the original report describes in this section.]
...
*Knoweldge attributions*
  ...
  Compared to the baseline model (AIC = `r AIC(model.1)`), a model with a random intercept for vignette (AIC = `r AIC(model.2)`) explained more variance. Psuedo Rsqaured values suggest that vignette accounts for `r round(r.squaredGLMM(model.2)[2,2]*100, 1)`% to `r round(r.squaredGLMM(model.2)[1,2]*100, 1)`% of the random variance in knowledge attributions. People tended to attribute knowledge differently in each vignette. People attributed Darrel's identification to his knowledge and his belief with roughly equal frequency, but slightly favored attributing his knowledge. People tended to attribute Emma's identification to her belief much more often than to her knowledge. People tended to attribute Gerald's identification to his belief slightly more often than to his knowledge. 
  
```{r}
#pretty results go here? If not, we need to enhance in-text reporting.
```

  A model with a random intercept for vignette nested within participant (AIC = `r AIC(model.3)`) explained similar amounts of variance to a model with a random intercept for vignette alone. Psuedo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(model.3)[2,2]*100, 1)`% to `r round(r.squaredGLMM(model.3)[1,2]*100, 1)`% of the random variance in knowledge attributions.
  Next, a model with a random intercept for vignette nested in participant nested in data collection site was estimated (AIC = `r AIC(model.4)`). The addition of data collection site was not useful relative to the previous model. Psuedo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(model.4)[2,2]*100, 1)`% to `r round(r.squaredGLMM(model.4)[1,2]*100, 1)`% of the random variance in knowledge attributions.
  Next, covariates were added to the model as fixed effects to account for the effects of participant age, compensation, gender, and education on their knowledge attributions (AIC = `r AIC(model.5)`). Relative to the model without covariates, this model was more useful in explaining variance in knowledge attribution. However, no covariates predicted knowledge attributions accounting for all others.
  
```{r}
#need to show more results...
```

  Finally, a model was estimated that included knowledge condition as a fixed effect (AIC = `r AIC(model.6)`). This model performed better than the previous model.
