...
*Reasonableness attributions*
  ...
  To determine whether participants made different judgments about vignette subjects' reasonableness as a funtion of the vignette's knowledge condition, we estimated a series of logistic regressions, just as with the previous analyses that focused on knowledge attribution as a dependent variable. Like with those analyses, we analyzed responses originally reported on continuous and binary scales together, using a variable where 0 means the subject was rated as reasonable and 1 means the subject was rated as unreasonable.  Listwise deletion was used to handle missing data. 
  A baseline intercept-only model was estimated initially and models with random intercepts for vignette, person_code, and data collection site were added sequentially, followed by a model with coviariates, and then a model with knowledge condition. Then, models with interactions were estimated to better understand whether the effect of condition on reasonableness differed as a function of vignette, sampling type, and measurement type.
  Compared to the baseline model (AIC = `r round(AIC(r.model.1), 2)`), a model with a random intercept for vignette (AIC = `r round(AIC(r.model.2), 2)`) explained more variance. For all vignettes, the great majority of people rated subjects as reasonable. However, the rates differed slighltly by vignette. Collapsing across conditions, more people rated Emma unreasonable than Gerald, and more people rated Gerald as unreasonable than Darrel. Psuedo Rsqaured values suggest that vignette accounts for a small portion of the random variance in reasonableness attributions (between `r round(r.squaredGLMM(r.model.2)[2,2]*100, 1)`% to `r round(r.squaredGLMM(r.model.2)[1,2]*100, 1)`%).
...
```
  ...
  A model with a random intercept for vignette nested within participant (AIC = `r round(AIC(r.model.3), 2)`) explained similar amounts of variance to a model with a random intercept for vignette alone. Psuedo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(r.model.3)[2,2]*100, 1)`% to `r round(r.squaredGLMM(r.model.3)[1,2]*100, 1)`% of the random variance in knowledge attributions.
  Next, a model with a random intercept for vignette nested in participant nested in data collection site was estimated (AIC = `r AIC(r.model.4)`). The addition of data collection site was not useful relative to the previous model. Psuedo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(r.model.4)[2,2]*100, 1)`% to `r round(r.squaredGLMM(r.model.4)[1,2]*100, 1)`% of the random variance in knowledge attributions.
...
ggplot(r.data_graph) +
  ...
  xlab("Vignette") + 
  ylab("Condition") +
  ggtitle("Figure X. Reasonablenss Judgments by Condition and Vignette")
```
...
*Luck*
  ...
  We estimated a series of logistic regressions. We analyzed responses originally reported on continuous and binary scales together, using a variable where 0 means the subject's identification was attributed to their luck and 1 means the subject's identification was attributed to their ability. Listwise deletion was used to handle missing data. 
...
```
  ...
  A model with a random intercept for vignette nested within participant (AIC = `r round(AIC(l.model.3), 2)`) explained similar amounts of variance to a model with a random intercept for vignette alone. Psuedo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(l.model.3)[2,2]*100, 1)`% to `r round(r.squaredGLMM(l.model.3)[1,2]*100, 1)`% of the random variance in knowledge attributions.
  Next, a model with a random intercept for vignette nested in participant nested in data collection site was estimated (AIC = `r AIC(l.model.4)`). The addition of data collection site was not useful relative to the previous model. Psuedo Rsqaured values suggest that this model accounts for `r round(r.squaredGLMM(l.model.4)[2,2]*100, 1)`% to `r round(r.squaredGLMM(l.model.4)[1,2]*100, 1)`% of the random variance in knowledge attributions.
...
In the Gerald vignette,the overall pattern was the same. People made different attributions in the different conditions X^2(`r l.gcond_effect$parameter[[1]]`) = `r round(l.gcond_effect$statistic[[1]], 2)`, p `r if(round(l.gcond_effect$p.value[[1]], 4) == 0)paste("<.0001")`. As with Darrel and Emma, the likelihood that people attributed Gerald's identification to his lcuk differed such that people were more likely to attribute luck in the Gettier condition (`r round(l.gcond_GI$estimate1, 2)`) than in the ignorance condition (`r round(l.gcond_GI$estimate2, 2)`; Cramer's `r l.gcond_GI_v$estimate`, `r l.gcond_GI_v$statistic`). In addition, people were less likely to attribute lcuk to Gerald in the knowledge condition (`r round(l.gcond_GK$estimate2, 2)`) than in the Gettier condition (Cramer's `r l.gcond_GK_v$estimate`, `r l.gcond_GK_v$statistic`). 
...
For the binary measurement type, there was an effect of condition on luck attributions  X^2(`r bin_cond$parameter[[1]]`) = `r round(bin_cond$statistic[[1]], 2)`, p `r if(round(bin_cond$p.value[[1]], 4) == 0)paste("<.0001")`. People were more likley to attribute subjects' identifications to luck in the Gettier condition (`r round(bin_cond_GI$estimate1, 2)`) than in the ignorance condition (`r round(bin_cond_GI$estimate2, 2)`; Cramer's `r bin_cond_GI_v$estimate`, `r bin_cond_GI_v$statistic`). People were also more likely to attribute subjects' identifications to luck in the Gettier condition (`r round(bin_cond_GK$estimate1, 2)`) than in the knowledge condition (`r round(bin_cond_GK$estimate2, 2)`; Cramer's `r bin_cond_GK_v$estimate`, `r bin_cond_GK_v$statistic`). 
