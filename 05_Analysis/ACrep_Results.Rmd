...
To determine whether participants made different attributions about vignette subject's knowledge as a funtion of the vignette's knowledge condition, we estimated a series of logistic regressions. We analyzed responses originally reported on continuous and binary scales together, using a variable where 0 means the subject's identification was attributed to their knowledge ("knows") and 1 means the subject's identification was not ("belief"). Listwise deletion was used to handle missing data. 
  First, a baseline intercept-only model was estimated. Then, models with random intercepts for vignette, person, and data collection site were added sequentially, before models with different interaction terms were estimated.
...
*Reasonableness attributions*
  ...
  A baseline intercept-only model was estimated initially and models with random intercepts for vignette, person, and data collection site were added sequentially, followed by a model with coviariates, and then a model with knowledge condition. Then, models with interactions were estimated to better understand whether the effect of condition on reasonableness differed as a function of vignette, sampling type, and measurement type.
